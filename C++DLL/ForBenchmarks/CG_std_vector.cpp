...
CaveGenerator_vector::CaveGenerator_vector(size_t width, size_t height, bool randInit) noexcept:
	CaveGeneratorBench(width, height, randInit)
{
	InitMatrix(randInit);
	name = "vector<vector<bool>>";
}

CaveGenerator_vector::CaveGenerator_vector(size_t width, size_t height, int threadsCount, bool randInit) noexcept:
...
CaveGenerator_vector::CaveGenerator_vector(size_t width, size_t height, vector<bool>& vec, int threadsCount) noexcept:
...
void CaveGenerator_vector::InitMatrix(bool randInit) noexcept
{
...
void CaveGenerator_vector::InitThreads(int threadsCount) noexcept
{
	_threadsCount = threadsCount;
}

int CaveGenerator_vector::GetNeighbours(size_t x, size_t y) const noexcept
{
	int sum = 0;

	if (x > 0 && x + 1 < _width && y > 0 && y + 1 < _height) {
		sum += _mainMatrix[x - 1][y - 1]; // верхний левый
		sum += _mainMatrix[x][y - 1];	  // верхний
		sum += _mainMatrix[x + 1][y - 1]; // верхний правый
		sum += _mainMatrix[x - 1][y];     // левый
		sum += _mainMatrix[x + 1][y];     // правый
		sum += _mainMatrix[x - 1][y + 1]; // нижний левый
		sum += _mainMatrix[x][y + 1];	  // нижний
		sum += _mainMatrix[x + 1][y + 1]; // нижний правый
	}
	else {
		if (x > 0) {
...
void CaveGenerator_vector::Tick(const int count) noexcept
{
...
void CaveGenerator_vector::TickMT(const int count) noexcept
{
...
void CaveGenerator_vector::TickMTRealization(const size_t lineFrom, const size_t lineTo) noexcept
{
	size_t x = lineFrom;
	for (; x < lineTo; x++) {
...
bool* CaveGenerator_vector::Data() noexcept
{
...
CaveGenerator_vector::~CaveGenerator_vector() noexcept {
	_width = 0;
	_height = 0;
	_threadsCount = 0;
	if (_dataPtr != nullptr) delete[] _dataPtr;
...
ostream& operator<<(ostream& stream, const CaveGenerator_vector& cave) noexcept
{
