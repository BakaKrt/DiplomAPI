...
export module OnlyAvxRealization;
...
export class TotalOnlyAvxRealization : public TotalRealizationBase<TotalOnlyAvxRealization> {
private:
...
#pragma region lambdas
		...
		auto saveRes = [&lutB, &lutS, &obj_ptr, &res_ptr] (__m256i& neighbours, __m256i& orig_row, size_t save_offset) noexcept {
			const __m256i one = _mm256_set1_epi8(1);

			__m256i res_B = _mm256_shuffle_epi8(lutB, neighbours);
			__m256i res_S = _mm256_shuffle_epi8(lutS, neighbours);
			__m256i live_mask = _mm256_cmpeq_epi8(orig_row, one);

			__m256i res = _mm256_blendv_epi8(res_B, res_S, live_mask);
			_mm256_store_si256(reinterpret_cast<__m256i*>(res_ptr + save_offset), res);
		};
		auto saveResNotFit = [&lutB, &lutS, &res_ptr, &remainder] (__m256i& neighbours, __m256i& orig_row, size_t save_offset) noexcept {
			const __m256i one = _mm256_set1_epi8(1);

			__m256i res_B = _mm256_shuffle_epi8(lutB, neighbours);
			__m256i res_S = _mm256_shuffle_epi8(lutS, neighbours);
			__m256i live_mask = _mm256_cmpeq_epi8(orig_row, one);
			__m256i result_full = _mm256_blendv_epi8(res_B, res_S, live_mask);

			alignas(32) uint8_t temp_buf[32] {};
			_mm256_store_si256(reinterpret_cast<__m256i*>(temp_buf), result_full);

			memcpy(res_ptr + save_offset, temp_buf, remainder);
		};
		...
		auto calcFirstTwoLines = [&top, &mid, &low, &res_ptr, &saveRes](__m256i* rightElement, size_t save_offset) -> __m256i {
			...
			*rightElement = verticalSum1;
			...
			neighbours0 = _mm256_sub_epi8(neighbours0, top[0]);
			saveRes(neighbours0, top[0], save_offset);
		...
		auto calcTwoLines = [&top, &mid, &low, &res_ptr, &saveRes] (__m256i leftVerticalResWithoutRightNeighbour, __m256i* leftVElem, size_t save_offset) noexcept -> __m256i {
			...
			*leftVElem = verticalSum1;
			...
			saveRes(leftTotalResult, top[0], save_offset);
		...
		auto calcTwoLastNotFitLines = [&top, &mid, &low, &res_ptr, &remainder, &saveResNotFit] (size_t save_offset) {
			...
			saveResNotFit(verticalSum, top[1], save_offset);
		};
		...
		auto calcFirstThreeLines = [&top, &mid, &low, &res_ptr, &saveRes] (__m256i* rightVSumElem, size_t save_offset) noexcept -> __m256i {
			...
			*rightVSumElem = verticalSum1;
			...
			neighbours0 = _mm256_sub_epi8(neighbours0, mid[0]);
			saveRes(neighbours0, mid[0], save_offset);
		...
		auto calcThreeLines = [&top, &mid, &low, &res_ptr, &saveRes] (__m256i leftVerticalResWithoutRightNeighbour, __m256i* leftVElem, size_t save_offset) -> __m256i {
			...
			*leftVElem = verticalSum1;
			...
			saveRes(leftTotalResult, mid[0], save_offset);
		...
		auto calcThreeLastNotFitLines = [&top, &mid, &low, &res_ptr, &remainder, &saveResNotFit] (size_t save_offset) {
			...
			saveResNotFit(verticalSum, mid[1], save_offset);
